    ...
    <RevealSection id="projects" ariaLabelledBy="projects-heading" className="landing-section">
      <div className="landing-section__inner">
        <div className="landing-section__header">
          <h2 id="projects-heading" style={{ fontSize: 'var(--font-size-section-title)', color: 'var(--color-text)' }}>
          ...
          {projects.map((project, index) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 'var(--space-2)' }}>
                <h3 style={{ fontSize: 'var(--font-size-title)', color: 'var(--color-text)' }}>{project.name}</h3>
                {project.period && (
                  <span style={{ fontSize: 'var(--font-size-caption)', color: 'var(--color-text-secondary)' }}>{project.period}</span>
                )}
              </div>
                ...
                <div style={{ marginBottom: 'var(--space-3)', flex: 1 }}>
                  ...
                  <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 'var(--space-1)' }}>
                    {project.highlights.map((line, i) => (
                      <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-1)', fontSize: 'var(--font-size-caption)', color: 'var(--color-text-secondary)' }}>
                        <FiCheck aria-hidden="true" size={13} style={{ marginTop: 2, flexShrink: 0, color: 'var(--color-accent)' }} />
                        {line[language]}
                      </li>
                    ))}
                  </ul>
