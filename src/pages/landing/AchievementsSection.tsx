...
import { FiCalendar, FiExternalLink, FiEye, FiStar } from 'react-icons/fi'
...
export function AchievementsSection() {
  ...
  const languageEntries = visible.filter((entry) => entry.group === 'language')
  const techEntries = visible.filter((entry) => entry.group === 'tech')

  function renderCard(entry: AchievementEntry) {
    const IssuerIcon = getBrandIcon(entry.issuer)
    return (
      <article
        key={entry.id}
        className={`landing-card${entry.featured ? ' landing-card--featured' : ''}`}
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-2)' }}>
          ...
          {entry.featured && (
            <span title={t('fields.featured')}>
              <FiStar aria-hidden="true" size={14} style={{ color: 'var(--color-accent)' }} />
              <span className="sr-only">{t('fields.featured')}</span>
            </span>
          )}
        </div>
      ...
      <div className="landing-section__inner">
        ...
        {languageEntries.length > 0 && (
          <div style={{ marginBottom: 'var(--space-8)' }}>
            <h3 className="landing-subheading" style={{ marginBottom: 'var(--space-4)' }}>
              {t('landing.achievements.groups.language')}
            </h3>
            <div className="landing-grid">{languageEntries.map(renderCard)}</div>
          </div>
        )}
        ...
        {techEntries.length > 0 && (
          <div>
            <h3 className="landing-subheading" style={{ marginBottom: 'var(--space-4)' }}>
              {t('landing.achievements.groups.tech')}
            </h3>
            <div className="landing-grid">{techEntries.map(renderCard)}</div>
          </div>
        )}
      </div>
